...
First, let's load our packages. You should already have the `here` and `ggplot2` packages installed; read them in from the library in your code. Also install and read in the `palmerpenguins` package, which contains built-in data.
...
Let's create a basic graph to begin, but plotting `bill_length_mm` on the x-axis and `bill_depth_mm` on the y-axis.
...
Next, 1) change the aesthetics of this plot to color the points by the species of penguin and 2) interpret the graph by describing if this changes your answer to the previous question.
